...
export const Route = createFileRoute("/learn/$courseId/$lessonId")({
  ...
  notFoundComponent: () => (
    <div className="min-h-screen flex items-center justify-center">
      <Link to="/courses" className="text-primary font-bold">← Quay lại</Link>
    </div>
  ),
});
...
function LearningPage() {
  ...
  const handleComplete = () => {
    ...
    navigate({ to: "/learn/$courseId/$lessonId/quiz", params: { courseId: course.id, lessonId: lesson.id } });
  };
    ...
    <div className="min-h-screen bg-background flex flex-col" key={tick}>
      ...
      <header className="sticky top-0 z-30 bg-background/90 backdrop-blur border-b border-border/50">
        <div className="flex h-14 items-center px-4 gap-3">
          ...
          >
            <ArrowLeft className="h-4 w-4" /> <span className="hidden sm:inline">{course.title}</span>
      ...
      <div className="flex-1 flex">
        ...
        <aside className={`${
          ...
          <div className="p-4">
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wide mb-1">{course.grade}</p>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-xl p-3 text-sm transition ${
                    active ? "bg-primary text-primary-foreground font-bold shadow-soft"
                      : "hover:bg-muted"
                  }`}
                >
                  <div className={`flex h-7 w-7 items-center justify-center rounded-lg shrink-0 ${
                    ...
                    {done ? <CheckCircle2 className="h-4 w-4 text-success" />
                      : <span className={`text-xs font-bold ${active ? "text-primary-foreground" : "text-muted-foreground"}`}>{i + 1}</span>}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="truncate">{l.title}</p>
                    <p className={`text-xs truncate ${active ? "text-primary-foreground/70" : "text-muted-foreground"}`}>{l.duration}</p>
                  </div>
                </Link>
              );
            })}
        ...
        {sidebarOpen && (
          <div onClick={() => setSidebarOpen(false)} className="lg:hidden fixed inset-0 top-14 z-10 bg-foreground/40" />
        )}
        ...
        <main className="flex-1 min-w-0 p-4 md:p-8 max-w-4xl mx-auto w-full">
          ...
          {/* Title */}
          <div className="mt-6">
            <p className="text-xs font-bold text-primary uppercase tracking-wide">Bài {index + 1} · {lesson.duration}</p>
          ...
          <div className="mt-8 rounded-2xl bg-gradient-primary p-6 text-center shadow-glow">
            ...
            <p className="font-display text-xl font-bold text-primary-foreground">Sẵn sàng làm quiz?</p>
            <p className="text-primary-foreground/80 text-sm mt-1">Trả lời {lesson.quiz.length} câu hỏi để hoàn thành bài học.</p>
            <button
