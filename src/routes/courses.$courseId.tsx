...
import { getCourse, type Lesson } from "@/lib/courses-data";
...
function CourseDetail() {
  ...
  const firstIncomplete =
    course.lessons.find((l: Lesson) => !isLessonComplete(course.id, l.id)) ?? course.lessons[0];
  ...
  return (
    <div className="min-h-screen flex flex-col" key={tick}>
      ...
      <main className="container mx-auto px-4 py-8 flex-1 max-w-5xl">
        <Link
          to="/courses"
          className="inline-flex items-center gap-1.5 text-sm font-bold text-muted-foreground hover:text-primary mb-6"
        >
        ...
        {/* Hero */}
        <div
          className={`rounded-3xl bg-gradient-to-br ${course.color} p-8 text-white shadow-glow relative overflow-hidden`}
        >
          <div className="absolute -right-6 -bottom-6 text-[10rem] opacity-20 leading-none">
            {course.emoji}
          </div>
          <div className="relative z-10">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="rounded-full bg-white/25 backdrop-blur px-3 py-1 text-xs font-bold">
                {course.grade}
              </span>
              <span className="rounded-full bg-white/25 backdrop-blur px-3 py-1 text-xs font-bold">
                {course.level}
        ...
        <div className="mt-8">
          ...
          <ul className="space-y-3">
            {course.lessons.map((l: Lesson, i: number) => {
              ...
              const Inner = (
                <div
                  className={`flex items-center gap-4 rounded-2xl p-4 border-2 transition ${
                    done
