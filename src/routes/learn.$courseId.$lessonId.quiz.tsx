...
import { getLesson, type QuizQuestion } from "@/lib/courses-data";
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-hero">
        <div className="w-full max-w-lg rounded-3xl bg-card p-8 shadow-glow text-center">
          <div
            className={`inline-flex h-20 w-20 items-center justify-center rounded-full mb-4 ${passed ? "bg-gradient-success" : "bg-warning/20"}`}
          >
            <Trophy
              className={`h-10 w-10 ${passed ? "text-success-foreground" : "text-warning-foreground"}`}
            />
          </div>
          ...
          <div className="mt-6 space-y-2 text-left">
            {questions.map((q: QuizQuestion, i: number) => {
              ...
              return (
                <div
                  key={i}
                  className={`rounded-xl p-3 text-sm ${ok ? "bg-success/10" : "bg-destructive/10"}`}
                >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {!ok && (
                        <p className="text-xs text-muted-foreground mt-0.5">
                          ...
                          · Đáp án đúng: <b className="text-success">{q.options[q.answer]}</b>
                        </p>
                      )}
                      {q.explanation && (
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-2">
            <button
              onClick={reset}
              className="flex-1 inline-flex h-12 items-center justify-center gap-2 rounded-xl border-2 border-border bg-card font-bold hover:border-primary transition"
            >
            ...
            {q.options.map((opt: string, i: number) => {
              ...
              return (
                <button
                  key={i}
                  onClick={() =>
                    setAnswers((arr) => arr.map((a, idx) => (idx === current ? i : a)))
                  }
