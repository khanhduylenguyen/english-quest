...
export const Route = createFileRoute("/signup")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Tạo tài khoản EngQuest miễn phí và bắt đầu cuộc phiêu lưu học tiếng Anh." },
    ],
  }),
        ...
        <div className="rounded-3xl bg-card border border-border/60 p-6 sm:p-8 shadow-glow">
          ...
          <div className="flex items-center justify-center gap-2 mb-6">
            {[1, 2, 3].map((n) => (
              <div key={n} className="flex items-center gap-2">
                <div className={`flex h-8 w-8 items-center justify-center rounded-full font-bold text-sm transition ${
                  step >= n ? "bg-gradient-primary text-primary-foreground shadow-soft" : "bg-muted text-muted-foreground"
                }`}>
                  {step > n ? <Check className="h-4 w-4" /> : n}
                </div>
                {n < 3 && <div className={`h-1 w-10 rounded-full ${step > n ? "bg-primary" : "bg-muted"}`} />}
              </div>
            ))}
          </div>
          ...
          {step === 1 && (
            <div>
              ...
              <p className="text-center text-sm text-muted-foreground mt-1">Bắt đầu cuộc phiêu lưu tiếng Anh nào! 🎒</p>
              <form onSubmit={(e) => { e.preventDefault(); setStep(2); }} className="mt-6 space-y-4">
                <Field label="Tên đăng nhập" placeholder="superstar2025" />
                <Field label="Email phụ huynh" type="email" placeholder="phuhuynh@example.com" hint="Để xác thực và gửi báo cáo học tập" />
                <Field label="Mật khẩu" type="password" placeholder="Ít nhất 6 ký tự" />
                <button type="submit" className="w-full h-12 rounded-xl bg-gradient-primary font-bold text-primary-foreground shadow-pop hover:translate-y-0.5 hover:shadow-none transition-all">
                  Tiếp theo →
                </button>
                <button type="button" className="w-full h-12 rounded-xl border-2 border-border bg-background font-bold text-sm hover:border-primary transition flex items-center justify-center gap-2">
          ...
          {step === 2 && (
            <div>
              ...
              <p className="text-center text-sm text-muted-foreground mt-1">Chúng tớ sẽ gợi ý bài tập phù hợp nhất.</p>
              ...
              <div className="mt-6 flex gap-3">
                <button onClick={() => setStep(1)} className="flex-1 h-12 rounded-xl border-2 border-border font-bold hover:border-primary transition">← Quay lại</button>
                <button
                  disabled={!grade}
                  onClick={() => setStep(3)}
          ...
          {step === 3 && (
            <div>
              <h2 className="font-display text-2xl font-bold text-center">Chọn người đồng hành 🐾</h2>
              <p className="text-center text-sm text-muted-foreground mt-1">Linh vật sẽ cùng bạn vượt qua mọi thử thách!</p>
              <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
                {MASCOTS.map((m) => (
                  <button
                    key={m.id}
                  ...
                  >
                    <img src={m.img} alt={m.name} width={512} height={512} loading="lazy" className="w-full aspect-square object-contain" />
              ...
              <div className="mt-6 flex gap-3">
                <button onClick={() => setStep(2)} className="flex-1 h-12 rounded-xl border-2 border-border font-bold hover:border-primary transition">← Quay lại</button>
                <button
                  disabled={!mascot}
