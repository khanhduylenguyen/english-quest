...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Đăng nhập vào tài khoản EngQuest để tiếp tục cuộc phiêu lưu tiếng Anh." },
    ],
  }),
  component: LoginPage,
});
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md animate-pop-in">
        ...
        <div className="rounded-3xl bg-card border border-border/60 p-8 shadow-glow">
          ...
          >
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center gap-2 font-semibold cursor-pointer">
                <input type="checkbox" checked={remember} onChange={(e) => setRemember(e.target.checked)} className="h-4 w-4 rounded accent-primary" />
                Ghi nhớ đăng nhập
              </label>
          ...
          <button className="w-full h-12 rounded-xl border-2 border-border bg-background font-bold text-sm hover:border-primary transition flex items-center justify-center gap-2">
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            Chưa có tài khoản?{" "}
            <Link to="/signup" className="font-bold text-primary hover:underline">Đăng ký ngay</Link>
          </p>
        </div>
      </div>
    </div>
